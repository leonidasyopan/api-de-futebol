...
function getTeamInfo(i) {
  ...
  output +=
    '<figure class="my-team-image"><img src="' +
    premierTeams[i].crestUrl +
    '" alt="' +
    premierTeams[i].name +
    ' Thumb"></figure>';
...
function getTeamPlayers() {
  ...
  output +=
    '<table id="table-squad"><thead><tr><th>Player Name</th><th>Position</th><th>Nationality</th><th>Shirt Number</th></tr></thead><tbody>';
  for (var i = 0; i < teamInfo.length; i++) {
    ...
    var playerNationalityURL = playerNationality
      .replace(/\s+/g, "-")
      .toLowerCase();
    ...
    output +=
      "<td>" +
      '<figure class="player-nationality-image"><img src="img/country-flags/' +
      playerNationalityURL +
      '.svg" alt="' +
      playerNationality +
      '"></figure>';
    +"</td>";
...
function displayStandings() {
  ...
  var standings_deserialized = JSON.parse(
    localStorage.getItem("PremierStandings")
  );
  ...
  output += "<section>";
  output +=
    '<table id="table-standings"><thead><tr><th>#</th><th></th><th>Club</th><th> P </th><th> W </th><th> D </th><th> L </th><th>Pts</th><th>GD</th></tr></thead><tbody>';
  for (var i = 0; i < premierStandings.length; i++) {
    output += "<tr><td>" + premierStandings[i].position + "</td>";
    output +=
      '<td><figure class="figure-standings"><img src="' +
      premierStandings[i].team.crestUrl +
      '" alt="' +
      premierStandings[i].team.name +
      ' Thumb"></figure></td>';
  ...
  output += "</tbody></table>";
  output +=
    '<p id="legends"><strong>P</strong> = Played, <strong>W</strong> = Won, <strong>D</strong> = Draw, <strong>L</strong> = Lost, <strong>Pts</strong> = Points</p>';
  output += "</section>";
...
function figureMatchday() {
  ...
  for (var i = 0; i < matchesList.length; i++) {
    ...
    var tomorrow = new Date(new Date().setDate(new Date().getDate() + 1))
      ...
      .slice(0, 10)
      .replace(/-/g, "/");
    var twoTomorrow = new Date(new Date().setDate(new Date().getDate() + 2))
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
    var threeTomorrow = new Date(new Date().setDate(new Date().getDate() + 3))
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
    var fourTomorrow = new Date(new Date().setDate(new Date().getDate() + 4))
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
    var fiveTomorrow = new Date(new Date().setDate(new Date().getDate() + 5))
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
    var sixTomorrow = new Date(new Date().setDate(new Date().getDate() + 6))
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
    var sevenTomorrow = new Date(new Date().setDate(new Date().getDate() + 7))
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
...
function displayMatches() {
  /* Requesting API and Storing JSON into Local Storage */
  var currentMatchDay = document.querySelector("#current-matchday")
    .classList[0];
  var url =
    "https://api.football-data.org/v2/competitions/2021/matches?matchday=" +
    currentMatchDay;
  ...
  output +=
    '<table id="table-matches"><thead><tr><th>Date/Time</th><th>Home</th><th></th><th></th><th>X</th><th></th><th></th><th>Away</th></tr></thead><tbody>';
  for (var i = 0; i < premierMatches.matches.length; i++) {
    ...
    var time = localDate.toLocaleString("en-US", {
      hour: "numeric",
      minute: "numeric",
      hour12: true,
    });
    ...
    output +=
      '<td><figure class="figure-matches"><img src="' +
      homeURL +
      '" alt="' +
      premierTeams.name +
      ' Thumb"></figure></td>';
